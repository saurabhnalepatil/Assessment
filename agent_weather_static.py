...
class Planner:
    """
    Very small planner that converts the natural-language prompt into steps.
    For this demo, it's intentionally simple and rule-based.
    """
    ...
    def parse_prompt(self, prompt):
        ...
        if m:
            days = int(m.group(1))
        location = None
        m2 = re.search(r'in\s+([A-Za-z \-]+?)(?:\,|\s+over|\s+and|\s*$)', prompt, re.IGNORECASE)
        if m2:
            location = m2.group(1).strip()
        if not location:
            location = "Paris"  # fallback for this task
        return {"location": location, "days": days, "action": "avg_temp_and_convert"}
